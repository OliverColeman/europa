...
/**
 * Mutator that adds neurons to a NEAT genotype. A neuron is added by replacing an existing synapse with a neuron that is connected to the same neurons the original synapse was connected to.
 * 
 * @author O. J. Coleman
 */
public class NEATNeuronAddMutator extends Mutator<NEATGenotype> {
	...
	public NEATNeuronAddMutator(ConfigurableComponent parentComponent, JsonObject componentConfig) throws Exception {
	...
	public void mutate(NEATGenotype genotype) {
		...
		for (int i = 0; i < maximum && i < synapseIDs.size(); i++) {
			...
			if (random.nextDouble() < applyRate) {
				...
				// Add the new neuron.
				NEATNeuronAllele newNeuron = evolver.newNeuronAllele(synapse.gene.id);
				genotype.addAllele(newNeuron);
				
				// Add the new pre and post synapses.
				NEATSynapseAllele preSynapseAllele = evolver.newSynapseAllele(synapse.gene.sourceID, newNeuron.gene.id);
				genotype.addAllele(preSynapseAllele);
				NEATSynapseAllele postSynapseAllele = evolver.newSynapseAllele(newNeuron.gene.id, synapse.gene.destinationID);
