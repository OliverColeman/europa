...
/**
 * Accepts command line arguments and launches a Run. Do something like <code>Java -jar [jarfile]</code> to see available options.
 * 
 * @author O. J. Coleman
 */
public class Main {
	...
	private void printConfigOptionsFromConfig(String configFile) throws Exception {
		...
		Base base = new Base(null, Json.parse(configStr).asObject());
		printConfig(base);
	}
	...
	private void printConfig(Base base) throws Exception {
		...
		if (!strictJSON) {
			// Comment out meta data. This produces totally illegal JSON. However they totally should have have included comments in the standard.
			configOut = Pattern.compile("^(\\s*)\"_metadata(?:.*?) ([^\"]*)\"\\s*:\\s*(.*?),?$", Pattern.MULTILINE).matcher(configOut).replaceAll("$1// $2: $3");
			configOut = Pattern.compile("^\\s*//\\s*:\\s*(\"\")?\\s*$", Pattern.MULTILINE).matcher(configOut).replaceAll(""); // Blank lines.
		}
