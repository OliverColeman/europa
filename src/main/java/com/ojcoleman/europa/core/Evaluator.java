...
public abstract class Evaluator extends ConfigurableComponent {
	 ...
	 */
	public Evaluator(ConfigurableComponent parentComponent, JsonObject componentConfig) throws Exception {
	...
	/**
	 * <p>Evaluate the given {@link Individual}s. The Evaluator must call
	 * {@link #setResult(EvaluationDescription, double)} on individual.evaluationData 
	 * for each evaluation type it defines in {@link #getEvaluationDescriptions()}.</p>
	 * <p><strong>It is critical that this method is thread-safe (can handle multiple simultaneous calls).</strong></p> 
	 */
